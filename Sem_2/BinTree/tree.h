...
struct tree {
    int data; // данных
    tree* left = 0; //левого элемента дерева
    tree* right = 0; //правого элемента дерева
    int x = 790, y = 0;
};
...
public:
    ...
    tree* insert(tree* root, int elem, int x, int y, int step) {
        if (root == nullptr) {
            root = new tree;
            root->data = elem;
            root->x = x;
        ...
        else if (elem < (int)(root->data)) {

           root->left = insert(root->left, elem, x-step, y+100, step/2);
        }
        else {
           root->right = insert(root->right, elem, x+step, y+100, step/2);
        }
    return root;
    }
    ...
    void printPreOrdered(tree* cur, QString &out) { //на вход - текущий элемент дерева
        out += QString::number(cur->data) + " ";
        if (cur->left != nullptr) { //если текущий элемент слева не пустой
            printPreOrdered(cur->left, out); //рекурсивно обрабатываем левое поддерево (выводим, у нас же сверху cout)
        }
        if (cur->right != nullptr) { //затем если текущий элемент справа не пустой
            printPreOrdered(cur->right, out); //рекурсивно выводим правое поддерево
        }
    }

    //обратный обход дерева (лево, корень, право) - по смыслу все то же самое, что выше
    void printPostOrdered(tree* cur, QString &out) {
        if (cur->left != nullptr) {
            printPostOrdered(cur->left, out);
        }
        if (cur->right != nullptr) {
            printPostOrdered(cur->right, out);
        }
        out += QString::number(cur->data) + " ";
    }

    //симметричный обход дерева (лево, право, корень) - по смыслу все то же самое, что выше
    void printInOrdered(tree* cur, QString &out) {
        if (cur->left != nullptr) {
            printInOrdered(cur->left, out);
        }
        out += QString::number(cur->data) + " ";
        if (cur->right != nullptr) {
            printInOrdered(cur->right, out);
        }
    }
